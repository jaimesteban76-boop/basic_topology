...
-- We need some code for subbasis..

def generate_from_subbasis (ℬ: Family X): Family X :=
  Set.sInter {𝒯 | ℬ ⊆ 𝒯 ∧ IsTopology 𝒯}

theorem subbasis_generates_topology (ℬ: Family X): IsTopology (generate_from_subbasis ℬ) := by
  sorry
